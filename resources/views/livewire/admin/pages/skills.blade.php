...
<div x-cloak x-data="{show: false}" class="w-full h-auto sm:h-full flex flex-col">
    ...
    <div class="flex flex-col md:flex-row mt-4 mx-4 md:justify-between md:w-[95%]" >
        <div class="flex flex-col mt-2 w-auto md:w-2/6 h-auto items-start md:ml-8 bg-slate-100 rounded-md md:py-1 p-0.5 md:pl-6 md:mb-6">
            <span class="text-base md:text-xl">Skills</span>
            <span class="text-xs md:text-sm">My Skills</span>
        </div>
        <div class="md:w-[95%] flex justify-end">
            <button @click.prevent="show = ! show" class="w-auto h-12 bg-MyBlue text-white rounded-lg px-3 py-1">Add skills</button>
        </div>
    </div>
    <div  class="w-full">
            ...
            <div class="w-11/12 md:w-3/6 flex flex-col justify-center bg-slate-50 rounded-md my-4 py-4 px-6 drop-shadow-lg">
                ...
                <form class="w-full" enctype="multipart/form-data">
                    ...
                    <div class="grid md:grid-rows-2 md:gap-6">
                        ...
                        <div class="relative z-0 mb-6 w-full">
                            <input type="text" id="level" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " wire:model="level" />
                            
                            <label for="level" class="peer-focus:font-medium absolute text-sm text-gray-500  duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-MyBlue-600 peer-focus:dark:text-MyBlue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">level</label>
                            @error('level') <span class="error text-xs text-red-600">{{ $message }}</span> @enderror
                        </div>
                    </div>
                       ...
                       <tr class="bg-white border-b">
                          <td class="py-4 px-6" wire:key="data-{{ $data['id'] }}">
                            ...
                            <td class="py-4 px-6" wire:key="data-{{ $data['id'] }}">
                                ...
                                {{$data['name']}}
                                @else
                                    <input type="text" wire:model.defer="skills.{{$index}}.name" class="mt-2 text-sm pl-2 pr-4 rounded-lg border border-spacing-1" enctype="multipart/form-data">
                                    @if($errors->has('skills.' . $index . '.name'))
                                        <div class="text-red">
                                            <span class="error text-xs">{{$errors->first('skills.' . $index . '.name')}}</span>    
                                        </div>    
                                    @endif
                                @endif
                            </td>
                                <td class="py-4 px-6" wire:key="data-{{ $data['id'] }}">
                                    ...
                                    {{$data['level']}}
                                    @else
                                        <input type="text" wire:model.defer="skills.{{$index}}.level" class="mt-2 text-sm pl-2 pr-4 rounded-lg border border-spacing-1">
                                        @if($errors->has('skills.' . $index . '.level'))
                                        <div class="text-red">
                                            <span class="error text-xs">{{ $errors->first('skills.' . $index . '.level') }}</span>    
                                        </div>    
                                    @endif
                                    @endif
                                </td>
                                <td class="py-4 px-6" wire:key="data-{{ $data['id'] }}">
                                    @if($editedIndex !== $index)
                                    <img src="{{URL::asset('/storage/'.$data['image'])}}" alt="Skill Image" class="object-fit w-8 h-8">
                                    @else
                                        <input type="file" name="image" wire:model="image" class="mt-2 text-sm pl-2 pr-4 rounded-lg border border-spacing-1">
                                    @endif
                                </td>
                          <td class="py-4 px-8">
                            @if($editedIndex !== $index)
