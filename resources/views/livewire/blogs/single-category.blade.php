...
@endphp
@section('title'){{$category->category.' Category'}}@endsection
    ...
    <div class="w-full flex flex-col lg:flex-row">
        ...
        <div class="flex flex-col items-center w-full h-auto mt-5 px-8 lg:px-10 ">
            <div class="bg-white w-full max-w-6xl h-30 mt-10 rounded-md">
                <div class="flex flex-col px-4 py-5 gap-y-2">
                    <span class="text-xl font-bold text-dark-blue"># {{$category->category}}</span>
                    <span class="font-plex text-slate-500 text-sm">{{$category->desc}}</span>
                </div>
            </div>
            {{-- Search Bar --}}
            ...
            @if(sizeof($blogs) == 0)
                @if ($search != '')
                    <div class="w-full h-full flex flex-col justify-center items-center" x-transition.duration.500ms>
                        <img src="{{URL::asset('/image/empty.svg')}}" alt="empty-box" class="max-h-96 min-h-60 my-10">
                        <p class="text-3xl md:text-5xl lg:text-4xl xl:text-5xl font-plex text-slate-300 ">{{__('No blogs matched')}}</p>
                        <p class="text-lg xl:text-2xl font-plex text-slate-600 mt-4">" {{$search}} " </p>
                    </div>
                @else
                <div class="w-full h-full flex flex-col justify-center items-center">
                    <img src="{{URL::asset('/image/empty.svg')}}" alt="empty-box" class="max-h-96 min-h-60 my-10">
                    <p class="text-3xl md:text-5xl lg:text-4xl xl:text-5xl font-plex text-slate-300 ">{{__('No blogs Listed')}}</p>
                </div>
                @endif
                
            @else 
                {{-- Filters --}}
                <div class=" {{ textDirection($locale) }} h-10 xl:h-16 w-full lg:w-11/12 max-w-6xl xl:mt-5 xl:mb-3 xl:ml-20 flex justify-start items-center font-plex">
        ...
        <div class="w-full lg:w-1/3 lg:h-fit flex flex-col lg:pr-8 items-center mb-10">
            ...
            <div class="h-auto grid grid-cols-2  grid-rows-auto bg-white gap-1 rounded-md p-2 md:p-3 drop-shadow-sm mb-10">
                ...
                @endphp
                    <a href="/category/{{$item->slug}}" class="{{$colorClass}} p-2 rounded-lg text-white flex justify-center items-center text-center hover:scale-105 transition ease-in-out">
